    ...
    public class LocalSuppliersService : ISuppliersService, IDisposable
    {
            ...
            )
        {
            ...
            var supplierDtoChangeSets = ObservableChangeSet.Create<SupplierDto, long>(
                ...
                ;
            this.Changes = supplierDtoChangeSets.RefCount();
            this.Cache = supplierDtoChangeSets
                //.Sort(SortExpressionComparer<SupplierDto>.Ascending(vm => vm.Name))
                //.RefCount()
        ...
        public async Task<IReadOnlyCollection<SupplierDto>> GetAllAsync()
        {
            ...
            var supplierDTOs = suppliers.Select(s => SupplierDto.From(s)).ToList();

            return supplierDTOs;
        }

        public IObservable<IChangeSet<SupplierDto, long>> Changes { get; }
        public IObservableCache<SupplierDto, long> Cache { get; }
        ...
        public async Task<SupplierDto> CreateAndAddAsync(IDictionary<string, object> data)
        {
            ...
            var addedSupplierDto = SupplierDto.From(addedSupplier);
        ...
        public async Task UpdateAsync(long supplierId, IDictionary<string, object> changes)
        {
            ...
            var addedSupplierDto = SupplierDto.From(updatedSupplier);
        ...
        //public IObservable<ChangeSet<SupplierDto>> Connect(IObservable<SupplierCriteria> supplierCriteriaChanged)
        //{
        //    return supplierCriteriaChanged
        //        .Select(criteria =>
        //        {

        //            ObservableChangeSet.Create<SupplierDto, long>(
        //            async cache =>
        //            {
        //            //await Task.Delay(3_000);
        //            var supplierDTOs = await this.GetAllAsync().ConfigureAwait(false);
        ...
        //                this.Added
        //                    .Select(async x => this.GetAllAsync)
        //                    .Subscribe(supplierDto => cache.Edit(cacheUpdater => cacheUpdater.AddOrUpdate(supplierDto)))
        //                    .DisposeWith(crudSubscriptions);
